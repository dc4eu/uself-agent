...
import com.eviden.bds.rd.uself.agent.services.repositories.authenticSource.AuthenticSourceRepositoryImp
...
class AuthenticSourceRepositoryImpTest(
    private val authenticSourceDAO: AuthenticSourceDAO
) : KoinTest, BehaviorSpec() {

    private val authenticSourceRepository = AuthenticSourceRepositoryImp(authenticSourceDAO)

    init {

        Context("an AuthenticSourceRepositoryImp") {
            ...
            Given("insert method") {
                `when`("a new authentic source is inserted") {
                    ...
                    then("it should save the authentic source in the database") {
                        val result = authenticSourceRepository.insert(id, source)
            ...
            Given("findById method") {
                `when`("the authentic source exists") {
                    ...
                    then("it should return the authentic source") {
                        val result = authenticSourceRepository.findById(id)
                        result shouldBe "source1"
                    }
                }
            ...
            Given("findAll method") {
                `when`("there are authentic sources in the database") {
                    ...
                    then("it should return a list of authentic sources") {
                        authenticSourceRepository.insert("2", "source2")
                        val result = authenticSourceRepository.findAll()
